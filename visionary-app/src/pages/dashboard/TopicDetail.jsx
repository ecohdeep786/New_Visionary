    ...
    <div className={focus ? "fixed inset-0 z-50 overflow-y-auto bg-white p-5 sm:p-8" : "mx-auto flex w-full max-w-[1280px] flex-col gap-6 p-5 sm:p-8"}>
      <div className={focus ? "mx-auto max-w-[1000px]" : ""}>
        ...
        <div className={focus ? "" : "grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_260px]"}>
          {topic.lab === "cube-volume" ? <section className="rounded-2xl border border-[#dadce0] bg-[#f8fafd] p-8"><h2 className="text-xl font-medium">Explore cube volume in 3D</h2><p className="mt-3 text-sm leading-6 text-[#5f6368]">Change the side length and see how volume changes in three dimensions.</p><Link to="/dashboard/explore" className="mt-5 inline-block rounded-full bg-[#1967d2] px-5 py-2.5 text-sm text-white">Open interactive lab</Link></section> : <CourseViewer key={topic.id} topic={topic} nextTopic={nextTopic} isBookmarked={bookmarks.some((item) => item.topic_id === topic.id)} bookmarkBusy={bookmarkBusy} onToggleBookmark={toggleBookmark} isFullscreen={focus} onToggleFullscreen={() => setFocus((value) => !value)} />}
