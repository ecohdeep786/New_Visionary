    ...
    <div className={focus ? "fixed inset-0 z-50 overflow-y-auto bg-white p-5 sm:p-8" : "mx-auto flex w-full max-w-[1280px] flex-col gap-6 p-5 sm:p-8"}>
      <div className={focus ? "mx-auto max-w-[1000px]" : ""}>
        ...
        <div className={focus ? "" : "grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_260px]"}>
          {topic.lab === "cube-volume" ? <section className="rounded-2xl border border-[#dadce0] bg-[#ffffff] p-8"><h2 className="text-xl font-medium">Explore cube volume in 3D</h2><p className="mt-3 text-sm leading-6 text-[#5f6368]">Change the side length and see how volume changes in three dimensions.</p><Link to="/dashboard/explore" className="mt-5 inline-block rounded-full bg-[#4285F4] px-5 py-2.5 text-sm text-white">Open interactive lab</Link></section> : <CourseViewer key={topic.id} topic={topic} nextTopic={nextTopic} isBookmarked={bookmarks.some((item) => item.topic_id === topic.id)} bookmarkBusy={bookmarkBusy} onToggleBookmark={toggleBookmark} isFullscreen={focus} onToggleFullscreen={() => setFocus((value) => !value)} />}
          {!focus && <aside className="flex flex-col gap-4">
            <section className="rounded-xl border border-[#dadce0] p-5"><h2 className="mb-4 text-sm font-medium">Keep learning</h2><div className="space-y-1">{[{ path: "ask", label: "Ask a question", icon: MessageCircle }, { path: "practice", label: "Practice this topic", icon: Target }, { path: "build", label: "Start a project", icon: Blocks }].map(({ path, label, icon: Icon }) => <Link key={path} to={`/dashboard/${path}?${context}`} className="flex items-center gap-3 rounded-lg px-2 py-3 text-sm text-[#5f6368] hover:bg-[#ffffff]"><Icon className="h-4 w-4 text-[#4285F4]" />{label}</Link>)}</div></section>
            {topics.length > 1 && <nav aria-label="Subject lessons" className="rounded-xl border border-[#dadce0] p-5"><h2 className="mb-3 text-sm font-medium">In this subject</h2><div className="max-h-80 space-y-1 overflow-y-auto">{topics.map((item) => <Link key={item.id} to={`/dashboard/learn/${item.id}`} aria-current={item.id === topic.id ? "page" : undefined} className={`block rounded-lg px-3 py-2 text-sm ${item.id === topic.id ? "bg-[#e8f0fd] font-medium text-[#4285F4]" : "text-[#5f6368] hover:bg-[#121317]/5"}`}>{item.name}</Link>)}</div></nav>}
          </aside>}
        </div>
      </div>
    </div>
