...
export default function Support() {
  ...
  ].map(([title,description,to,label]) => <section key={title} className="rounded-2xl border border-[#dadce0] p-6"><h2 className="text-base font-medium">{title}</h2><p className="mt-3 text-sm leading-7 text-[#5f6368]">{description}</p><Link to={to} className="mt-4 inline-block text-sm text-[#1967d2]">{label}</Link></section>)}</div>;
}
