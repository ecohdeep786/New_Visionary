...
export default function Ask() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1000px] flex-col gap-6 p-5 sm:p-8">
      <header className="flex items-start justify-between gap-4"><div><h1 className="text-2xl font-medium tracking-tight text-[#121317]">Ask</h1><p className="mt-2 text-sm text-[#5f6368]">A place for your questions, ideas, and next steps.</p></div>{messages.length > 0 && <button disabled={loading} onClick={() => { setMessages([]); setError(""); setInput(""); }} className="inline-flex items-center gap-2 rounded-full border border-[#dadce0] px-4 py-2 text-sm font-medium text-[#5f6368] disabled:opacity-40"><Plus className="h-4 w-4" /> New chat</button>}</header>
      {topic && <div className="flex flex-wrap items-center gap-2 rounded-lg bg-[#ffffff] px-4 py-3 text-sm text-[#5f6368]"><BookOpen className="h-4 w-4 shrink-0" /><span className="min-w-0 break-words">{subject} · {topic}</span><Link to={`/dashboard/learn?subject=${encodeURIComponent(subject)}`} className="ml-auto text-[#4285F4] hover:underline">View lessons</Link></div>}
      {messages.length ? <section aria-label="Conversation" className="space-y-6 rounded-xl border border-[#dadce0] bg-white p-5 sm:p-7">{messages.map((message, index) => <div key={index} className={message.role === "user" ? "ml-auto max-w-[90%] rounded-2xl bg-[#dadce0] p-4" : "min-w-0"}>
        {message.role === "assistant" && <div className="mb-3 flex items-center gap-2 text-sm font-medium"><Sparkles className="h-4 w-4 text-[#4285F4]" /> Visionary</div>}
        <div className="break-words text-sm leading-7 text-[#5f6368]">{message.role === "user" ? <p className="whitespace-pre-wrap">{message.content}</p> : <ReactMarkdown className="prose prose-sm max-w-none [&_pre]:overflow-x-auto">{message.content}</ReactMarkdown>}</div>
      </div>)}{loading && <p role="status" className="flex items-center gap-2 text-sm text-[#5f6368]"><Loader2 className="h-4 w-4 animate-spin" /> Working on your question…</p>}<div ref={endRef} /></section> : <section className="rounded-xl border border-[#dadce0] bg-white px-5 py-10 text-center sm:py-14"><div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-2xl" style={{ backgroundColor: theme.light, color: theme.accent }}><MessageCircle className="h-7 w-7" /></div><h2 className="text-2xl font-medium text-[#121317]">What are you curious about, {name}?</h2><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-[#5f6368]">Start with a question in your own words. You can save it for your next study session.</p><div className="mx-auto mt-7 grid max-w-xl gap-2 text-left">{suggestions.map((suggestion) => <button key={suggestion} onClick={() => { setInput(suggestion); setNotice(""); }} className="flex items-center gap-3 rounded-xl border border-[#dadce0] px-4 py-3 text-sm text-[#5f6368] hover:bg-[#ffffff]"><Lightbulb className="h-4 w-4 shrink-0 text-[#4285F4]" /><span className="flex-1">{suggestion}</span><ArrowRight className="h-4 w-4 shrink-0 text-[#5f6368]" /></button>)}</div></section>}
      <form onSubmit={askQuestion} className="rounded-2xl border border-[#dadce0] bg-white p-4 focus-within:border-[#4285F4]">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3"><button type="button" onClick={saveQuestion} disabled={!input.trim() || saving} className="inline-flex items-center gap-2 rounded-full px-3 py-2 text-sm text-[#5f6368] hover:bg-[#121317]/5 disabled:opacity-40"><Bookmark className="h-4 w-4" />{saving ? "Saving…" : "Save question"}</button><button type="submit" disabled={loading || !input.trim() || data.loading} className="inline-flex items-center gap-2 rounded-full px-5 py-2 text-sm font-medium text-white disabled:opacity-40" style={{ backgroundColor: theme.accent }}><Send className="h-4 w-4" />{loading ? "Sending…" : "Ask Visionary"}</button></div>
      </form>
      {error && <p role="alert" className="rounded-xl border border-[#dadce0] bg-[#ffffff] p-4 text-sm leading-6 text-[#5f6368]">{error}</p>}
      {notice && <p role="status" className="flex items-center gap-2 text-sm text-[#137333]"><Check className="h-4 w-4" />{notice}</p>}
      {questions.length > 0 && <section className="rounded-xl border border-[#dadce0] bg-white"><div className="border-b border-[#dadce0] px-5 py-4"><h2 className="text-base font-medium text-[#121317]">Saved questions <span className="ml-2 text-sm font-normal text-[#5f6368]">{questions.length}</span></h2></div><div className="divide-y divide-[#dadce0]">{questions.map((question) => <div key={question.id} className="flex items-center gap-3 p-4 sm:px-5"><Bookmark className="h-4 w-4 shrink-0 text-[#5f6368]" /><Link to={`/dashboard/ask?${new URLSearchParams({ subject: question.subject || "", topic: question.topic || "" })}`} state={{ initialQuestion: question.question }} className="min-w-0 flex-1 hover:text-[#4285F4]"><p className="break-words text-sm">{question.question}</p>{question.subject && <p className="mt-1 text-xs text-[#5f6368]">{question.subject}{question.topic ? ` · ${question.topic}` : ""}</p>}</Link><button onClick={() => removeQuestion(question.id)} className="rounded-full p-2 text-[#5f6368] hover:bg-[#121317]/5" aria-label={`Remove saved question: ${question.question}`}><Trash2 className="h-4 w-4" /></button></div>)}</div></section>}
    </div>
  );
}
