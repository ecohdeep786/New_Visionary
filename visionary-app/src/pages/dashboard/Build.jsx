  ...
  return (
    <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-6 p-5 sm:p-8">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div><h1 className="text-2xl font-medium tracking-tight text-[#121317]">Build</h1><p className="mt-2 text-sm text-[#5f6368]">Turn what you learn into something of your own.</p></div>
      ...
      <section className="rounded-xl border border-[#dadce0] bg-white">
        ...
        {loading ? <div role="status" className="flex items-center justify-center gap-3 py-24 text-sm text-[#5f6368]"><Loader2 className="h-5 w-5 animate-spin" /> Loading projects</div> : error && !draft ? <div role="alert" className="p-6 text-sm text-[#b3261e]">{error} <button onClick={loadProjects} className="underline">Try again</button></div> : visible.length ? <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">{visible.map((project) => {
          ...
          return <article key={project.id} className="flex flex-col rounded-xl border border-[#dadce0] p-5">
            <div className="mb-5 flex items-center justify-between gap-3"><div className="rounded-xl p-3" style={{ backgroundColor: theme.light, color: theme.accent }}><Blocks className="h-5 w-5" /></div><span className={`rounded-md px-2 py-1 text-xs ${project.status === "completed" ? "bg-[#e6f4ea] text-[#137333]" : "bg-[#e8f0fd] text-[#4285F4]"}`}>{project.status === "completed" ? "Completed" : "In progress"}</span></div>
            ...
            {topic && <Link className="mt-4 inline-flex items-center gap-1 text-xs text-[#4285F4] hover:underline" to={`/dashboard/learn/${topic.id}`}>Lesson: {topic.name}</Link>}
        ...
        })}</div> : <div className="flex flex-col items-center gap-3 px-5 py-16 text-center"><div className="rounded-2xl bg-[#dadce0] p-4"><Blocks className="h-8 w-8 text-[#5f6368]" /></div><h2 className="text-lg font-medium">{query || filter !== "all" ? "No projects match your filters" : "Big ideas start with a small project"}</h2><p className="max-w-md text-sm leading-6 text-[#5f6368]">{query || filter !== "all" ? "Try another search or view all your projects." : "Create a project notebook, connect it to a lesson, and keep your plan and discoveries together."}</p>{query || filter !== "all" ? <button onClick={() => { setQuery(""); setFilter("all"); }} className="text-sm font-medium text-[#4285F4]">Clear filters</button> : <button onClick={newProject} className="mt-2 rounded-full border border-[#dadce0] px-5 py-2 text-sm font-medium text-[#4285F4]">Create your first project</button>}</div>}
      </section>
      <Dialog open={!!draft} onOpenChange={(open) => { if (!open) setDraft(null); }}><DialogContent className="max-h-[90vh] max-w-[calc(100vw-2rem)] overflow-y-auto rounded-2xl sm:max-w-xl"><DialogTitle>{draft?.id ? "Project notebook" : "New project"}</DialogTitle><DialogDescription>Capture your idea, plan your steps, and record what you learn.</DialogDescription>{draft && <form onSubmit={saveProject} className="space-y-4">
        ...
        {error && <p role="alert" className="text-sm text-[#b3261e]">{error}</p>}<div className="flex justify-end gap-3"><button type="button" onClick={() => setDraft(null)} className="rounded-full px-4 py-2 text-sm">Cancel</button><button disabled={saving || !draft.title.trim()} className="rounded-full bg-[#4285F4] px-5 py-2 text-sm font-medium text-white disabled:opacity-40">{saving ? "Saving…" : "Save project"}</button></div>
