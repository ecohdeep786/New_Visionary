...
export default function DecisionHome() {
  ...
  let transition = null;
  try { transition = getActiveTransitionNotice(ctx); } catch { transition = null; }
  ...
  return <div className={`v-page v-home stage-${stageTier}`}>
    ...
    {transition && <section className="v-home-module" aria-labelledby="stage-transition-title"><p className="v-home-priority-label">Your stage</p><h2 id="stage-transition-title">{transition.state === 'awaiting-confirm' ? 'Confirm your new stage' : 'Your stage changed'}</h2><p className="v-muted">Now: {transition.to.classLevel || 'not set'}{transition.to.board ? ` · ${transition.to.board}` : ''} · {transition.diff.planStepsKept} plan steps and {transition.diff.unitsKept} learning activities kept{transition.diff.openClassworkKept ? ` · ${transition.diff.openClassworkKept} open classwork items` : ''}. Due dates are retained.</p>{transition.state === 'awaiting-confirm' && <div className="v-home-actions"><button type="button" className="v-button primary" onClick={() => { try { confirmStageTransition(ctx, transition.id); } catch { /* the notice refreshes with the failure state */ } }}>Confirm</button></div>}{transition.state === 'applied' && <div className="v-home-actions"><button type="button" className="v-button" onClick={() => { try { undoStageTransition(ctx, transition.id); } catch (e) { } }}>Undo this change</button><button type="button" className="v-button" onClick={() => { try { postponeStageTransition(ctx, transition.id); } catch (e) { } }}>Postpone 7 days</button></div>}{transition.state === 'postponed' && <p className="v-muted">Reevaluation scheduled for {transition.postponedUntil ? new Date(transition.postponedUntil).toLocaleDateString() : ''}.</p>}</section>}
    <div className="v-home-modules">{modules.map(module=><section className="v-home-module" key={module.id} aria-labelledby={`home-${module.id}`}><h2 id={`home-${module.id}`}>{module.title}</h2>{module.rows.map(row=><div className="v-home-row" key={row.id}><div><p className="v-home-row-title">{row.title}</p><p className="v-muted">{row.detail}</p></div><div className="flex items-center gap-2">{row.deferId&&<button type="button" className="v-button" onClick={()=>{try{deferPlanStep(ctx,row.deferId);}catch(e){}}}>Not today</button>}<Link className="v-button" to={row.action.path}>{row.action.label}<span className="sr-only">: {row.title}</span></Link></div></div>)}</section>)}</div>
