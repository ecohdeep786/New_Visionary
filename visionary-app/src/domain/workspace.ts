...
export interface Message { id: string; role: 'user' | 'guide'; blocks: GuideBlock[]; at: string; status?: 'ready' | 'not_connected' | 'blocked' | 'mentor' | 'model' }
...
export interface Artifact { id: string; title: string; body: string; journeyId?: string; conceptId?: string; learningSessionId?: string; milestones: boolean[]; visibility: 'private' | 'shared'; sharedWith: string[]; versions: { body: string; at: string }[]; status: 'draft' | 'in-progress' | 'completed'; updatedAt: string }
export interface Resource { id: string; title: string; kind: 'lesson' | 'cohort' | 'goal' | 'curriculum' | 'listing'; body: string; status: string; audience: string; updatedAt: string; conceptId?:string; members?:string[]; classIds?:string[]; checks?:{id:string;prompt:string}[] }
