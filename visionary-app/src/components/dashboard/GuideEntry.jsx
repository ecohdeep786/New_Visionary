...
export default function GuideEntry({role,value: saved=defaultAskContext,onChange: persist,compact=false}) {
  ...
  const fields=<div className="guide-entry">
    ...
    {value.source==='material' && <label className="guide-entry-label">Material to keep with this conversation<textarea className="v-field" rows={3} maxLength={6000} value={value.material} onChange={e=>onChange({...value,material:e.target.value})}/><span className="v-muted">Text only. This demo saves material but does not analyze arbitrary documents.</span></label>}
