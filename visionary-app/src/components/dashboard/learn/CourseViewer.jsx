  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-xl border border-[#dadce0] bg-white">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[#dadce0] px-4 py-3">
        <h2 className="flex min-w-0 items-center gap-2 text-sm font-medium text-[#121317]"><BookOpen className="h-4 w-4 shrink-0" /><span className="truncate">{isFullscreen ? topic.name : "Lesson"}</span></h2>
        <div className="flex items-center gap-1">
          {audioSupported && lesson && <button aria-label={playing ? "Stop reading" : "Read lesson aloud"} title={playing ? "Stop reading" : "Read aloud"} onClick={toggleAudio} className="rounded-full p-2 text-[#5f6368] hover:bg-[#121317]/5">{playing ? <Square className="h-4 w-4" /> : <Headphones className="h-4 w-4" />}</button>}
          ...
          <button aria-label={isFullscreen ? "Exit focus mode" : "Enter focus mode"} title={isFullscreen ? "Exit focus mode (Esc)" : "Focus mode"} onClick={onToggleFullscreen} className="rounded-full p-2 text-[#5f6368] hover:bg-[#121317]/5">{isFullscreen ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}</button>
        </div>
      </header>
      <div className="min-h-[280px] p-5 sm:p-8">
        {loading ? <p role="status" className="flex items-center justify-center gap-3 py-16 text-sm text-[#5f6368]"><Loader2 className="h-5 w-5 animate-spin" /> Preparing an explanation</p> : lesson ? <ReactMarkdown className="prose prose-sm max-w-none break-words text-[#5f6368] [&_pre]:overflow-x-auto">{lesson}</ReactMarkdown> : <div className="flex flex-col items-center gap-3 py-10 text-center"><BookOpen className="h-8 w-8 text-[#5f6368]" /><h3 className="text-base font-medium">Your topic is ready to explore</h3><p className="max-w-md text-sm leading-6 text-[#5f6368]">Published lesson content will appear here when available. Keep a question, connect with your class, or request an explanation.</p><button onClick={generateLesson} className="mt-2 inline-flex items-center gap-2 rounded-full border border-[#dadce0] px-5 py-2 text-sm font-medium" style={{ color: theme.accent }}><Sparkles className="h-4 w-4" /> Request explanation</button></div>}
        {error && <p role="alert" className="mt-4 rounded-lg border border-[#dadce0] bg-[#ffffff] p-4 text-sm leading-6 text-[#5f6368]">{error}</p>}
      </div>
      <div className="flex flex-wrap items-center gap-3 border-t border-[#dadce0] p-4">
        <Link to={`/dashboard/practice?${context}`} className="rounded-full border border-[#dadce0] px-4 py-2 text-sm font-medium text-[#4285F4]">Practice this topic</Link>
        <Link to={nextTopic ? `/dashboard/learn/${nextTopic.id}` : `/dashboard/learn?subject=${encodeURIComponent(topic.subject)}`} className="ml-auto rounded-full px-4 py-2 text-sm font-medium text-[#4285F4] hover:bg-[#e8f0fd]">{nextTopic ? "Next topic →" : "All lessons →"}</Link>
      </div>
      <form onSubmit={saveQuestion} className="border-t border-[#dadce0] p-5"><label htmlFor="lesson-question" className="mb-3 block text-sm font-medium">Keep a question for later</label><div className="flex items-center gap-2 rounded-lg border border-[#dadce0] p-2"><input id="lesson-question" maxLength={6000} value={question} onChange={(e) => { setQuestion(e.target.value); setSaved(false); }} placeholder="What would you like to understand?" className="min-w-0 flex-1 bg-transparent px-2 text-sm outline-none" /><button disabled={saving || !question.trim()} title="Save question" aria-label="Save question" className="rounded-full p-2 text-white disabled:opacity-40" style={{ backgroundColor: theme.accent }}><Send className="h-4 w-4" /></button></div>{saved && <p role="status" className="mt-3 flex items-center gap-2 text-sm text-[#137333]"><Check className="h-4 w-4" /> Question saved. <Link to="/dashboard/ask" className="underline">View in Ask</Link></p>}</form>
    </section>
  );
}
