    ...
    <div className="flex gap-2 overflow-x-auto pb-1" aria-label="Filter by subject">
      {subjects.map((subject) => (
        <button key={subject.id || subject.name} onClick={() => onSelect(subject.name)} aria-pressed={activeSubject === subject.name}
          className="shrink-0 rounded-lg border px-4 py-2 text-sm font-medium transition-colors hover:bg-[#121317]/5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
