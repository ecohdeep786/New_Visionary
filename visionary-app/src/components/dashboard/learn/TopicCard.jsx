  ...
  return (
    <article className={`rounded-xl border border-[#dadce0] bg-white p-5 ${variant === "list" ? "sm:flex sm:items-center sm:gap-5" : "flex flex-col"}`}>
      <div className="mb-4 flex items-center justify-between gap-3 sm:shrink-0">
        ...
        {variant !== "list" && <span className="rounded-md bg-[#dadce0] px-2 py-1 text-xs text-[#5f6368]">{statuses[topic.status] || "Not started"}</span>}
      </div>
      <div className="min-w-0 flex-1">
        <p className="mb-1 text-xs text-[#5f6368]">{topic.chapter || topic.subject}</p>
        <h3 className="text-base font-medium leading-6 text-[#121317]"><Link className="hover:underline" to={`/dashboard/learn/${topic.id}`}>{topic.name}</Link></h3>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3 border-t border-[#dadce0] pt-4 sm:shrink-0">
        ...
        <Link to={`/dashboard/ask?${context}`} className="rounded-lg p-2 text-[#5f6368] hover:bg-[#121317]/5" aria-label={`Ask about ${topic.name}`} title="Ask"><MessageCircle className="h-4 w-4" /></Link>
        <Link to={`/dashboard/practice?${context}`} className="rounded-lg p-2 text-[#5f6368] hover:bg-[#121317]/5" aria-label={`Practice ${topic.name}`} title="Practice"><Target className="h-4 w-4" /></Link>
      </div>
    </article>
  );
}
