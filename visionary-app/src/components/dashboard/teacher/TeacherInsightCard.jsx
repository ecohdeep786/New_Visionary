...
export default function TeacherInsightCard({ accent, classes = [], assignmentCount = 0, pendingReviews = 0 }) {
  ...
  return <div className="flex items-start gap-4 rounded-2xl border border-[#dadce0] bg-[#ffffff] p-6"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white"><ClipboardCheck className="h-5 w-5" style={{ color: accent }} /></div><div className="min-w-0"><h2 className="text-sm font-medium text-[#121317]">Your teaching overview</h2><p className="mt-2 text-sm leading-relaxed text-[#5f6368]">{message}</p>{classes[0] && <Link to={`/dashboard/class/${classes[0].id}`} className="mt-3 inline-flex items-center gap-2 text-sm font-medium" style={{ color: accent }}>Open class <ArrowRight className="h-4 w-4" /></Link>}</div></div>;
}
