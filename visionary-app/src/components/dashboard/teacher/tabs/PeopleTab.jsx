  ...
  return (
    <div className="flex flex-col gap-6 max-w-[680px]">
      ...
      <div className="p-6 bg-white rounded-3xl border border-[#dadce0]/60">
        <p className="text-sm font-medium text-[#121317] mb-1">Invite students</p>
        <p className="text-sm text-[#5f6368] mb-4">Add an in-app invitation by email, or share class code <span className="font-mono font-medium text-[#121317]">{classroom?.join_code || "from the class header"}</span>. Email delivery is not connected yet.</p>
        <form onSubmit={invite} className="flex flex-col gap-3 sm:flex-row">
          <div className="flex-1 flex items-center gap-2 h-11 px-4 rounded-xl border border-[#dadce0] focus-within:border-[#4285F4]">
            ...
            <input
              ...
              className="min-w-0 flex-1 bg-transparent text-sm text-[#121317] placeholder:text-[#5f6368] outline-none"
            />
          </div>
          <button
            type="submit"
      ...
      <div className="bg-white rounded-3xl border border-[#dadce0]/60 overflow-hidden">
        <div className="px-6 py-4 border-b border-[#dadce0]/60">
          <h3 className="text-sm font-medium text-[#121317]">Teachers</h3>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-[#dadce0]/60 overflow-hidden">
        <div className="px-6 py-4 border-b border-[#dadce0]/60 flex items-center justify-between">
          <h3 className="text-sm font-medium text-[#121317]">Students</h3>
          ...
          <div className="divide-y divide-[#dadce0]/40">
            {enrollments.map((e) => (
              <div key={e.id} className="flex items-center gap-4 px-6 py-4">
                ...
                <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${e.status === "active" ? "bg-[#e6f4ea] text-[#137333]" : "bg-[#ffffff] text-[#5f6368]"}`}>{e.status === "active" ? "Connected" : "Invited"}</span>
              </div>
            ))}
          </div>
        )}
