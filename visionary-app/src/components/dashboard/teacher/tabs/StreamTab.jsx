...
export default function StreamTab({ classId, classroom, accent }) {
  ...
  return <div className="flex max-w-[800px] flex-col gap-6">
    <form onSubmit={post} className="flex flex-col gap-4 rounded-2xl border border-[#dadce0] bg-white p-5 sm:p-6">
      <label htmlFor="class-announcement" className="text-sm font-medium text-[#121317]">Share an update with your class</label>
      <textarea id="class-announcement" value={text} onChange={(e) => setText(e.target.value)} placeholder="Announcements, reminders, or a welcome message…" rows={4} maxLength={10000} className="w-full resize-y rounded-lg border border-[#dadce0] p-3 text-sm leading-relaxed text-[#121317] outline-none focus:border-[#4285F4]" />
      <div className="flex flex-wrap justify-between gap-3"><button type="button" disabled={Boolean(text)} onClick={() => setText(`Welcome to ${classroom?.name || "our class"}! You’ll find assignments in Classwork and class updates here. I’m looking forward to learning together.`)} className="inline-flex h-10 items-center gap-2 rounded-full border border-[#dadce0] px-4 text-sm font-medium text-[#5f6368] hover:bg-[#ffffff] disabled:opacity-40"><FileText className="h-4 w-4" />Welcome template</button><button type="submit" disabled={busy || !text.trim()} className="inline-flex h-10 items-center gap-2 rounded-full px-5 text-sm font-medium text-white disabled:opacity-50" style={{ backgroundColor: accent }}><Send className="h-4 w-4" />{busy ? "Posting…" : "Post update"}</button></div>
    </form>
    {error && <p role="alert" className="text-sm text-[#b3261e]">{error} <button onClick={load} className="underline">Retry</button></p>}
    {loading ? <p role="status" className="py-10 text-center text-sm text-[#5f6368]">Loading updates…</p> : announcements.length === 0 ? <div className="py-12 text-center"><Megaphone className="mx-auto mb-3 h-10 w-10 text-[#5f6368]" /><h3 className="font-medium text-[#121317]">Keep your class in the loop</h3><p className="mt-2 text-sm text-[#5f6368]">Post a welcome message to get started.</p></div> : announcements.map((a) => <article key={a.id} className="rounded-2xl border border-[#dadce0] bg-white p-6"><h3 className="text-sm font-medium text-[#121317]">{a.author_name || classroom.teacher_name || "Teacher"}</h3><p className="mt-1 text-xs text-[#5f6368]">{a.createdAt ? new Date(a.createdAt).toLocaleString([], { dateStyle: "medium", timeStyle: "short" }) : "Class update"}</p><p className="mt-4 whitespace-pre-wrap text-sm leading-relaxed text-[#5f6368]">{a.text}</p></article>)}
  </div>;
}
