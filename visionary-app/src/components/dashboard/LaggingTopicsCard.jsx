      ...
      {topics.slice(0, 4).map((t) => (
        <Link key={t.id} to={`/dashboard/learn/${t.id}`} className="flex items-center justify-between gap-8 py-5 group">
          <div className="flex flex-col min-w-0 flex-1">
            <span className="text-base font-medium text-[#121317] group-hover:text-[#4285F4] transition-colors truncate">{t.name}</span>
            <span className="text-sm font-normal text-[#5f6368] mt-1">{t.subject}</span>
          </div>
          <div className="flex items-center gap-6 shrink-0">
            <div className="flex items-center gap-2">
              <div className="w-20 h-[4px] bg-[#121317]/5 rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${t.mastery < 35 ? "bg-[#fce8e6]0" : "bg-[#ffffff]0"}`} style={{ width: `${t.mastery || 0}%` }} />
              </div>
              <span className={`text-sm font-medium w-8 text-right ${t.mastery < 35 ? "text-red-500" : "text-[#b06000]"}`}>{t.mastery || 0}%</span>
            </div>
            <div
