...
export function getContentRepository(ctx: RequestContext): ContentRepository {
 ...
 return {
  async getSyllabus(board, classLevel, subject) {
   ...
   check(ctx); const db = read(); const current = space(db, ctx); const matching = current.graphs.filter(g => same(g.syllabus, query)); const existing = matching.find(g => g.syllabus.status === 'official') ?? matching[0];
   if (graph) {
  ...
  async getChapters(syllabusId) { check(ctx); let graph = space(read(), ctx).graphs.find(g => g.syllabus.id === syllabusId); if (!graph) throw new Error('Syllabus unavailable in this workspace.'); if (graph.syllabus.id === 'sample:math' && graph.syllabus.status === 'sample') graph = sample(ctx.locale); return structuredClone(graph.syllabus.chapters); },
  async getTopics(chapterId) { check(ctx); let graph = space(read(), ctx).graphs.find(g => g.syllabus.chapters.some(c => c.id === chapterId)); if (!graph) throw new Error('Chapter unavailable in this workspace.'); if (graph.syllabus.id === 'sample:math' && graph.syllabus.status === 'sample') graph = sample(ctx.locale); return structuredClone(graph.topics.filter(t => t.chapterId === chapterId)); },
  async getConcepts(topicId) { check(ctx); let graph = space(read(), ctx).graphs.find(g => g.topics.some(t => t.id === topicId)); if (!graph) throw new Error('Topic unavailable in this workspace.'); if (graph.syllabus.id === 'sample:math' && graph.syllabus.status === 'sample') graph = sample(ctx.locale); return structuredClone(graph.concepts.filter(c => c.topicId === topicId && eligible(ctx, c))); },
  async getConcept(conceptId) {
   ...
   if (!concept && adapter?.getConcept) concept = await abortable(adapter.getConcept(id, ctx), ctx.signal) ?? undefined;
  ...
  async mapProvisional(provisionalId, officialId) {
   ...
   if (!next && adapter?.getConcept) next = await abortable(adapter.getConcept(officialId, ctx), ctx.signal) ?? undefined;
   check(ctx);
