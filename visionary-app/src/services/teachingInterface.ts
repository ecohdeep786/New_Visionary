...
export function getTeachingInterface(ctx: RequestContext): TeachingInterface {
 ...
 async function request(mode: TeachingMode, packet: PromptPacket): Promise<TeachingResponse> {
  ...
  if (packet.audience === 'adult' && identity.person.ageBand !== 'adult') return { status: 'blocked', source: 'safety', text: 'This activity is not available for this age profile. Choose a general learning activity.' };
  ...
  check(ctx);
  if (!result || !['ready', 'not_connected', 'blocked'].includes(result.status) || typeof result.text !== 'string') throw new Error('The teaching service returned an incomplete response. Your saved work is unchanged.');
