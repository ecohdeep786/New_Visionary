...
function eventFor(ctx: RequestContext, input: InteractionInput): InteractionEvent {
 if (!['LEARN', 'ASK', 'PRACTICE', 'BUILD', 'COMMUNITY'].includes(input.app) || !['view', 'start', 'request', 'response', 'answer', 'save', 'complete', 'resume', 'delete_memory', 'remove', 'report'].includes(input.action)) throw new Error('Unknown interaction type.');
